<template>
  <el-form :model="editForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
    ...
    <el-form-item label="伙伴公司" prop="name">
      <el-input v-model.number="editForm.name" autocomplete="off"></el-input>
    </el-form-item>

    <el-form-item label="描述" prop="description">
      <el-input v-model.number="editForm.description" autocomplete="off"></el-input>
    </el-form-item>
    ...
    export default {
      ...
      data() {
        var checkPartnerName = (rule, value, callback) => {
          if (value === '') {
            return callback(new Error('伙伴公司名称不能为空'));
          } else {
            callback();
          }
        };
          ...
          this.$axios
            ...
            .then(successResponse => {
              ...
              if (successResponse.data.code === 100) {
                //添加成功！
                this.$alert('添加成功！');
                this.$router.replace({path: '/partnerList'})
              }else {
